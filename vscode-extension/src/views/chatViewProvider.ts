...
import { chatWithBackend } from "../beClient";
import { applyPatchToFile } from "../git";
import { ChatMessage } from "../types";
import { WorkspaceContextRetriever } from "../workspaceContext";
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  public async handleUserMessage(promptText: string): Promise<void> {
    ...
    const replyText =
      res?.reply ??
      `### 🛡️ CodeGuardian AI Assistant\n\nAnalyzed active workspace at \`${context.repositoryPath}\`.\n\n` +
        `*(Backend sidecar is running locally. Set your OpenAI/GenAILab API Key in VS Code Settings \`codeguardian.apiKey\` for deep LLM reasoning).*`;
  ...
  private async applyPatchToActiveFile(code: string): Promise<void> {
    ...
    const relativePath = editor.document.fileName;

    const applied = await applyPatchToFile(rootPath, relativePath, editor.selection.start.line + 1, editor.document.getText(editor.selection), code);

    if (applied) {
      vscode.window.showInformationMessage("Applied code patch to active file!");
    } else {
  ...
  private getHtmlForWebview(nonce: string): string {
    const chatHtml = this._messages.length
          ...
          ${
            !isUser && index === this._messages.length - 1
              ? `<div class="followup-chips">
                  ...
                  <button class="btn-action btn-prompt" data-prompt="Generate unit tests for active file">🧪 Generate Tests</button>
                  <button class="btn-action btn-prompt" data-prompt="Refactor for security and performance">⚡ Refactor Code</button>
                 </div>`
              : ""
          }
        </div>
      ...
      <div class="welcome-card">
        ...
        <p>Ask anything about your codebase, active file, staged diffs, or architecture.</p>
...
<head>
  ...
  <style>
    ...
    .followup-chips { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 8px; }
    .chip { background: rgba(255,255,255,0.06); border: 1px solid var(--vscode-widget-border, #444); color: var(--vscode-sideBar-foreground); padding: 4px 8px; font-size: 10px; border-radius: 12px; cursor: pointer; }
    .chip:hover { background: var(--vscode-button-hoverBackground); color: #fff; }
    ...
    .btn-prompt { background: var(--vscode-button-secondaryBackground, #3a3d41); color: var(--vscode-button-secondaryForeground, #fff); border: none; padding: 7px 10px; font-size: 11px; border-radius: 5px; cursor: pointer; text-align: left; }
    ...
    .btn-send { background: var(--vscode-button-background); color: #fff; border: none; padding: 8px 16px; font-size: 12px; border-radius: 5px; cursor: pointer; font-weight: 600; }
  </style>
</head>
<body>
